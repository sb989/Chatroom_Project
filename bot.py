...
class Bot:
    ...
    def aboutMessage(self):
        about_str = "I am {}. Use me to translate," "send pictures, and more!".format(
            self.name
        )
        return about_str
    ...
    def commandList(self):
        ...
        for i in range(len(self.commands)):
            return_str += "!! " + self.commands[i] + ": " + descriptions[i] + "\n"
        return return_str
    ...
    def funTranslate(self, message):
        ...
        translated_mess = translateToRandomLang(p_id, message)
    ...
    def imageSearch(self, query):
        try:
            query = query.replace(" ", "+")
            url = "https://pixabay.com/api/?key={}&q={}&image_type=photo".format(
                self.image_id, query
            )
    ...
    def __init__(
        self, project_id, image_id, google_json, img, stm="!!", name="ChatBot"
    ):
